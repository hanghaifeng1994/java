...
<div class="bs-example" > 
	<ul class="nav nav-tabs nav-justified" id="tab" style="padding: 0px;">
        <li><a style="cursor: pointer;" href="${ctx }/count/manage/learningsituation!diqu.action">地区学情统计</a></li>
        <li class="active"><a style="cursor: pointer;" href="${ctx }/count/manage/learningsituation!diquhighchart.action">地区图表统计</a></li>
    </ul>                     
</div>   
...
<s:if test="currentTenant == null ">
	<div class="form-group">
					...
					headerKey="" cssClass="form-control input-sm" name="tenantId" onChange="loadHighchart(4,1);"></s:select>
					</div>
</s:if>
<div class="form-group">
	  	<label>所属城市</label>
...
function loadHighchart(type,htype,typename){
	...
	if(htype ==1){
	    ...
	    chart = new Highcharts.Chart({    
            chart: {    
		        ...
		        series: loadDiqu24HourData(typename)
		    }); 
	}else if(htype ==4){  
	    ...
	    chart = new Highcharts.Chart({
            ...
            series: loadDiquDateHourData(typename)
        }); 
	}
}
    ...
    function loadDiquDateHourData(typename){
    	var sdata = new Array(); 
		$.post("${ctx }/count/manage/learningsituation!loadDiquDateHourData.excsec?typeName="+typename+"&startTime="+Date.parse($("#startTime").val())+"&endTime="+Date.parse($("#endTime").val()),function(data) {
	...
	function loadDiqu24HourData(typename){
		var sdata = new Array(); 
		$.post("${ctx }/count/manage/learningsituation!loadDiqu24HourData.excsec?typeName="+typename,function(data) {
			var arrayObj = data.toString().split(",");　
			$(arrayObj).each(function(index){   
				sdata.push(parseInt(arrayObj[index]));  
			});
	...
	function initData(){ 
	 	$.post("${ctx }/count/manage/learningsituation!getdiquHighchartData.excsec",{cityid:$("#city").val(),tenantId:$("#tenantId").val()},function(data) {
