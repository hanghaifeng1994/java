...
function change(){
    ...
    if(imageArray.indexOf(postfix)!=-1){
    	...
    	if(typeof src == "object") {//IE10 support
        	var preViewButton = '<a id="fancyPreView" rel="example_group" href=""><input id="buttonClick" type="button" class="btn" value="预览文件" /></a>';
        	$("#up").parents('.input-append').append(preViewButton);
            var reader = new FileReader();
            reader.onload = function(evt){$("#fancyPreView").attr("href", evt.target.result);};
            reader.readAsDataURL(src);
        } else {//IE7/8/9 support
        	var preViewButton = '<a id="fancyPreView" rel="example_group" href="#data"  kesrc="#data"><input id="buttonClick" type="button" class="btn" value="预览文件" /></a>'
