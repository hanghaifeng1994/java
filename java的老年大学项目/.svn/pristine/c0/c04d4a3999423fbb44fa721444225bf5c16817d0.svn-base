...
public class TestingPaperRuleWyService extends WeyeBaseService<TestingPaperRule> {
	...
	public TestingPaperRule detail(String id) {
		TestingPaperRule rule = super.queryById(id);
		TestingQuestionPool qp = testingQuestionPoolWyService.get(rule.getQpId());
		rule.setQpName(qp.getName());
		return rule;
	}
