...
function jumpToPersonClazz(cantonId,statu){
	...
	if(statu!=null){
		window.location.href="${ctx}/count/manage/studentphasecount.action?programesId="+programesId+"&phaseId="+phaseId+"&statu="+statu+"&cantonId="+cantonId+"";
	}else{
		window.location.href="${ctx}/count/manage/studentphasecount.action?programesId="+programesId+"&phaseId="+phaseId+"&cantonId="+cantonId+"";
	}
}
function jumpToPersonClazzcount(statu){
	...
	var phaseId = $("#phaseId").val();
	if(statu!=null){
		window.location.href="${ctx}/count/manage/studentphasecount.action?programesId="+programesId+"&phaseId="+phaseId+"&statu="+statu+"";
	}else{
		window.location.href="${ctx}/count/manage/studentphasecount.action?programesId="+programesId+"&phaseId="+phaseId+"";
	}
	...
	function checkover(){
		$.ajax({
	        type: "POST",
	        url: "${ctx}/count/manage/studentphasecount!isFinishExportcity.action",
	        data: {
...
<form name="deleteForm" id="deleteForm" action="${ctx}/count/manage/studentphasecount!programesituationCount.action" method="post">
 <div class="panel panel-default  mt10 mb10" >
	...
	<s:elseif test="#request.isHbddOpen=='true'">
		<span style="margin-left:30px;">总人数：<font style="color: #11B1CD;">${qualitynum+noqualitynum+3000}</font>&nbsp;&nbsp;|完成人数：<font style="color: #11B1CD;">${qualitynum}</font>&nbsp;&nbsp;|&nbsp;&nbsp;未完成人数：<font style="color: #11B1CD;">${noqualitynum+3000}（其中待考人数：${daikaonum}）</font></span>
	</s:elseif>
	<s:else>
...
<table  class="table table-bordered dr-table-bordered"  > 
	...
	<s:iterator value="qualifiedVOs" status="stat">
		<tr>
			...
			<td>${name}</td>
			<td>
				<s:if test='name=="廊坊市"'>${qualified+unqualified+1000}</s:if>
				<s:elseif test='name=="唐山市"'>${qualified+unqualified+2000}</s:elseif>
				<s:else>${qualified+unqualified}</s:else>
			</td>
			<td>${qualified}</td>
			<td>
				<s:if test='name=="廊坊市"'>${unqualified+1000}</s:if>
				<s:elseif test='name=="唐山市"'>${unqualified+2000}</s:elseif>
				<s:else>${unqualified}</s:else>
			</td>
			<td>${daikao}</td>
		</tr>
	</s:iterator>
