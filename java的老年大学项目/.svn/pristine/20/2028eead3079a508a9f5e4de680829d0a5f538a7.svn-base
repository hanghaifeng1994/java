...
<script type="text/javascript">
	...
	function changeprogram(){				
		...
		if(programesId) {
			$.post("${ctx}/count/manage/studentphasecount!getPhases.excsec",{"programesId":programesId},function(data) {
	...
	function changecity(){				
		var cityId = $("#cityId").val();
		if(cityId) {
			$.post("${ctx}/count/manage/studentphasecount!getAreaJson.excsec",{"cityId":cityId},function(data) {
				$("#cantonId option[value!='']").remove();
				var practiceUnit = eval(data);
				if(practiceUnit.length == 0) $("#cantonId option[value!='']").remove();
				for(var i = 0 ;i < practiceUnit.length ; i++) {
	...
	function checkover(){
		$.ajax({
	        type: "POST",
	        url: "${ctx}/count/manage/studentphasecount!isFinishExportNew.action",
	        data: {
...
<!--正文左边开始-->
	...
	<jsp:param value="studentphasecount" name="menu" />
...
<!--信息提示 end-->
<form class="form-inline dr-form-inline" id="mainForm" name="mainForm" action="${ctx}/count/manage/studentphasecount.action" method="post">
...
<s:if test="currentTenant == null ">
	<div class="form-group">
		<label>租户</label>
		<s:select cssStyle="width:150px"   list="tenantLists" listKey="id"  onchange="changeTenant()"
...
<div class="form-group">
	  	<label>所属项目</label>
	  	<s:select cssStyle="width:150px"   list="programesLists" onchange="changeprogram()" listKey="id" value="programesId" id="programesId" listValue="name" theme="simple"  cssClass="form-control" name="programesId"></s:select>
</div>

<div class="form-group">
	  	<label>所属阶段</label>
	  	<s:select cssStyle="width:150px"  list="phaseLists" listKey="id" value="phaseId" listValue="name" id="phaseId" theme="simple" headerValue="--全部--"
			headerKey="0"  cssClass="form-control" name="phaseId"></s:select>
</div>

<div class="form-group">
	  	<label>所属城市</label>
	  	<s:select cssStyle="width:150px" list="citysVLists" onchange="changecity()" listKey="id" listValue="name" id="cityId" name="cityId" value="cityId" theme="simple" cssClass="form-control" ></s:select>
</div>

<div class="form-group">
	  	<label>所属区域</label>
	  	<s:select cssStyle="width:150px"   cssClass="form-control" list="areaVLists" listKey="id" listValue="name" id="cantonId" name="cantonId" theme="simple" value="cantonId" headerKey="" headerValue="--所属区域--" />
</div>
...
<div class="form-group">
     <label>状态</label>
     <s:select   list="#{'1':'合格','0':'不合格','2':'待考'}" listKey="key" listValue="value" name="statu" theme="simple" headerValue="--全部--" headerKey="" value="@java.lang.Integer@parseInt(#parameters.statu)" cssClass="form-control" ></s:select>
</div>
</s:if>
<s:else>
<div class="form-group">
     <label>状态</label>
     <s:select   list="#{'1':'完成','0':'未完成','2':'待考'}" listKey="key" listValue="value" name="statu" theme="simple" headerValue="--全部--" headerKey="" value="@java.lang.Integer@parseInt(#parameters.statu)" cssClass="form-control" ></s:select>
</div>
</s:else >	
<!--  
<s:if test="type==0">
<div class="form-group">
     <label>是否合格</label>
     <s:select   list="#{'1':'是','0':'否'}" listKey="key" listValue="value" name="qualified" theme="simple" headerValue="--全部--" headerKey="" value="@java.lang.Integer@parseInt(#parameters.qualified)" cssClass="form-control" ></s:select>
   </div>
</s:if>
...
<div class="form-group">
     <label>作业是否提交</label>
     <s:select list="#{'1':'是','0':'否'}" listKey="key" listValue="value" name="homesignuped" theme="simple" headerValue="--全部--" headerKey="" value="@java.lang.Integer@parseInt(#parameters.homesignuped)" cssClass="form-control" ></s:select>
   </div>
 	...
 	<div class="btn-toolbar dr-btn-toolbar">
		       <button name="Submit2" type="button" class="btn btn-default btn-sm" id="exportbut" 
			    ...
			    <label class="ml10" id="downLoad" style="display: none">
					<a href="" target="_blank">专技个人阶段学情导出文件下载</a>
				</label>
			    </span>
			    
	<s:if test="#request.isahstudyopen=='true'||#request.isHbddOpen=='true'">
		<span style="margin-left:30px;">总人数：<font style="color: #11B1CD;">${qualitynum+noqualitynum}</font>&nbsp;&nbsp;|完成人数：<font style="color: #11B1CD;">${qualitynum}</font>&nbsp;&nbsp;|&nbsp;&nbsp;未完成人数：<font style="color: #11B1CD;">${noqualitynum}（其中待考人数：${daikaonum}）</font></span>
	</s:if>
	<s:else>
		<span style="margin-left:30px;">总人数：<font style="color: #11B1CD;">${qualitynum+noqualitynum}</font>&nbsp;&nbsp;|合格人数：<font style="color: #11B1CD;">${qualitynum}</font>&nbsp;&nbsp;|&nbsp;&nbsp;不合格人数：<font style="color: #11B1CD;">${noqualitynum}（其中待考人数：${daikaonum}）</font></span>
	</s:else>
</div>
...
<table  class="table table-bordered dr-table-bordered"  > 
	...
	<th width="5%">序号</th>
		...
		<s:if test="programsCheckDTO.hashour">		 
			<th width="8%">所得学时</th>
	...
	<s:iterator value="page.result" status="stat">
		...
		-->
			...
			<s:if test="programsCheckDTO.hasscore">	
				<s:if test='qualified=="1"'>合格</s:if>
			  	<s:elseif test='qualified=="0"'>不合格<s:if test='daikao=="1"'>(待考)</s:if></s:elseif>
			  	<s:else>未知</s:else>
			 </s:if>
			 <s:else >			 
			 	<s:if test='qualified=="1"'>完成</s:if>
			  	<s:elseif test='qualified=="0"'>未完成<s:if test='daikao=="1"'>(待考)</s:if></s:elseif>
			  	<s:else>未知</s:else>	  	
			 </s:else>
			</td>
			<%-- <td><s:property value="#stat.index+1"></s:property></td>
