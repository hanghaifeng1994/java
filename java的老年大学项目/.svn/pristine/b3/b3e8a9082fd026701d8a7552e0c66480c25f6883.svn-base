...
<div class="bs-example" > 
	<ul class="nav nav-tabs nav-justified" id="tab">
        ...
        <li  class="active"><a style="cursor: pointer;" href="${ctx }/count/graphics/studycountgraphics!areaclazzcomplete.action">地区班级合格率</a></li>
   </ul>                
</div> 
...
<div class="form-group" <s:if test="cityId!=null">style="display: none;"</s:if> >
	  	<label>所属城市</label>
	  	<s:select list="citysVLists" onchange="changecity();" listKey="id" listValue="name" id="cityId" name="cityId" value="cityId" theme="simple"  headerKey="" cssClass="form-control" ></s:select>
</div>


<div class="form-group" <s:if test="areaId!=null">style="display: none;"</s:if> >
	  	<label>所属地区</label>
	  	<s:select cssClass="form-control" onchange="loadHighchart();" list="areaVLists" listKey="id" listValue="name" id="areaId" name="areaId" theme="simple" value="areaId"   />
</div>

    <div class="form-group">
	  	<label>数据说明：班级合格率=班级合格人数/班级报名人数	</label>
	</div>
...
function loadHighchart() {
	arrayValue = initData(); 
	var clazzLength = getJsonLength(arrayValue.clazzName);
	
	var containerHeight = clazzLength*40 < 550 ? 550 : clazzLength*40;
    ...
    $('#container').highcharts({
        ...
        xAxis: [{
            ...
            categories:arrayValue.clazzName
        }],
		 plotOptions: {
                bar: {  
                    dataLabels: {
                ...
                column: {
                    ...
                    pointPadding: 0.2,
                    borderWidth: 0,
                   // pointWidth: 40//控制柱体的宽度 
                    pointWidth: 30//控制柱体的宽度
                }  
            },
        yAxis: [{ 
            min:0, 
...
function initData(){ 
 	$.post("${ctx }/count/graphics/studycountgraphics!getdiquJieyeHighchartData.excsec",{cityId:$("#cityId").val(),areaId:$("#areaId").val(),filter:'${filter}',tenantId:'${currentTenant.id}'},function(data) {
 		arrayValue.clazzName =  data.diqu;
