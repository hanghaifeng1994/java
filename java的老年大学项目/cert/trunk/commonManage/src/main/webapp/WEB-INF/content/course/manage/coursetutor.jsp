...
//提交删除,这里的删除不是真的删除辅导老师对象而是只是将其从该门课程关系去掉，不被加入这么课程的辅导老师
function ajaxDelete(id){
	...
	$('#coursetutor_container').load('${ctx}/course/manage/tutor!list.action?tutorUserIds='+tutorIds, false);
...
<center>
	...
	function ajaxAddTutors(tutorUserIds){
		...
		$.ajax({
	         type: "POST",
	         url: "${ctx}/course/manage/tutor!save.excsec",
	         data: {
	         ...
	         success: function(data) {
			 	var data = eval(data);
			 	if(data.value=="true"){
					$('#coursetutor_container').load('${ctx}/course/manage/tutor.excsec?tutorUserIds='+$("#tutorIds").val(), false);
			 	}else{
					alert(data.label);
			 	}
	         },
	...
	<iframe src="${ctx}/course/manage/tutor!tutorUsers.action?tutorUserIds=${tutorUserIds}&id=${id}" frameborder="0" scrolling="no" width="680" id="subMainFrame" name="subMainFrame"></iframe>
    </div>
</center>
