...
<title>商品分类管理<s:if test="currentTenant!=null">-${currentTenant.logo}</s:if><s:else><%@ include file="/common/title.jsp" %></s:else></title>
...
<div class="dr-wrapper">
	...
	<!--breadcrumb-->
		...
		<li class="active"><span>商品分类管理</span></li>
		</ol>
	<!--/breadcrumb-->
	
	<!--页面标题-->
        <div class="dr-page-header">
            <h3>商品分类管理<small>&nbsp;</small></h3>
        </div>
        <hr></hr>
    <!--页面标题end-->
<!--信息提示-->
	    ...
	    <div class="panel panel-default">
			...
			<table class="table table-bordered dr-table-bordered">
				...
				<tr>
					...
					<td class="white_bg">
					<s:if test="level>1">
						<a class="btn btn-primary btn-sm" href="${ctx }/cat/manage/category!moveUp.action?parentId=${parentId}&id=${id }&sortnum=${orderNum}&isMy=${isMy}">
		           			<span class="glyphicon glyphicon-arrow-up"></span>&nbsp;上移
		           		</a>	
						<a class="btn btn-default btn-sm"  href="${ctx }/cat/manage/category!moveDown.action?parentId=${parentId}&id=${id }&sortnum=${orderNum}&isMy=${isMy}">
			                 <span class="glyphicon glyphicon-arrow-down"></span>&nbsp;下移
			            </a>	
						<a class="btn btn-primary btn-sm" href="${ctx }/cat/manage/category!input.action?id=${id}&parentId=${parentId}&isMy=${isMy}"  type="button">
		                    <span class="glyphicon glyphicon-edit"></span>&nbsp;编辑
		                </a>	
					</s:if> 
					<s:if test="level<3">
					<%-- <a href="${ctx }/cat/manage/category.action?parentId=${id}">进入下级栏目</a> --%>
						<a class="btn btn-primary btn-sm" href="${ctx }/cat/manage/category.action?parentId=${id}&isMy=${isMy}" type="button" >
							<span class="glyphicon glyphicon-forward"></span>&nbsp;进入下级栏目
						</a>
					</s:if>
					<a class="btn btn-default btn-sm"  type="button" href="#"
						onclick="delRecord('${ctx }/cat/manage/category!delete.action?id=${id}&parentId=${parentId}&isMy=${isMy}');">
...
<script>
		...
		function folderOpen(parentId){
			...
			$.ajax({
		         type: "POST",
		         url: "${ctx}/cat/manage/category!folderOpen.action",
		         data: {
		       	 	"parentId":parentId
				 },
