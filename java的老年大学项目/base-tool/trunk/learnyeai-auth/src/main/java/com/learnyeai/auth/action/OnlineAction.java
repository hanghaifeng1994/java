...
import com.learnyeai.auth.domain.vo.OnlineUserVo;
...
//    		UserEntity user = (UserEntity) session.getAttribute(ShiroProperties.ATTRIBUTE_SESSION_CURRENT_USER);
			Account user = (Account) session.getAttribute(ShiroProperties.ATTRIBUTE_SESSION_CURRENT_USER);
    		if(null!=user){
    			onlineUser.setUserName(user.getAccount());
    		}else{
