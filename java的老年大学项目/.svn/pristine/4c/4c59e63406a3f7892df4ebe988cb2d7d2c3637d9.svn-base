...
**************************/
	...
	while(k-->0){
		...
		if(src.indexOf('LodopFuncs.js')!=-1){
			ctx=script.text;
			break;
		}
	}
	
    var strHtmInstall="<br><font color='#FF00FF'>打印控件未安装!点击这里<a href='"+ctx+"/js/install_lodop32.exe'>执行安装</a>,安装后请刷新页面或重新进入。</font>";
    var strHtmUpdate="<br><font color='#FF00FF'>打印控件需要升级!点击这里<a href='"+ctx+"/js/install_lodop32.exe'>执行升级</a>,升级后请重新进入。</font>";
    var strHtm64_Install="<br><font color='#FF00FF'>打印控件未安装!点击这里<a href='"+ctx+"/js/install_lodop64.exe'>执行安装</a>,安装后请刷新页面或重新进入。</font>";
    var strHtm64_Update="<br><font color='#FF00FF'>打印控件需要升级!点击这里<a href='"+ctx+"/js/install_lodop64.exe'>执行升级</a>,升级后请重新进入。</font>";
	...
	try{		     
		...
		if (navigator.userAgent.indexOf('Win64')>=0){
	     ...
	     //*****如下空白位置适合调用统一功能:*********	     
	     LODOP.SET_LICENSES("上海汇和信息技术有限公司","452596380837383919278901905623","","");
