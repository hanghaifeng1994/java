...
<div class="dr-wrapper">
	...
	<jsp:param value="cantonCount" name="menu" />
...
<div class="bs-example" > 
	<ul class="nav nav-tabs nav-justified" id="tab" style="padding: 0px;">
        <li><a style="cursor: pointer;" href="${ctx }/count/manage/learningsituation.action">地区学情统计</a></li>
        <li class="active"><a style="cursor: pointer;" href="${ctx }/count/manage/learningsituation!highchart.action">地区图表统计</a></li>
    </ul>                
</div> 
<form class="form-inline dr-form-inline" id="mainForm" name="mainForm"  method="post">
...
<s:if test="currentTenant == null ">
	  	...
	  	<label>所属项目</label>
	  	<s:select list="programesLists" listKey="id" value="programesId" listValue="name" theme="simple" headerValue="--选择项目--"
					headerKey="" cssClass="form-control" name="programesId" onChange="loadHighchart(4,1);"></s:select>
			
			<label>所属城市</label>
	  	<s:select id="city" list="citysLists" listKey="id" value="cityid" listValue="name" theme="simple" 
			headerKey="" cssClass="form-control" name="cityid" onChange="loadHighchart(4,1);"></s:select>
	    &nbsp; 单日时间设置：    
...
<div class="form-group" id="highchart_type" style="float: left">  
	    ...
	    <button class="btn btn-sm " type="button"   onclick="loadHighchart(6,3,'总地区')" id="highchart_6">24小时人数</button>
		<button class="btn btn-sm " type="button"   onclick="loadHighchart(7,4,'总地区')" id="highchart_7">单日学习时长</button>
</div>        
...
function loadHighchart(type,htype,typename){
	...
	if(htype ==1){
	    ...
	    chart = new Highcharts.Chart({    
            chart: {    
		        ...
		        series: loadDiquProgrames24HourData(typename)
		    }); 
	}else if(htype ==4){  
	    ...
	    chart = new Highcharts.Chart({
            ...
            series: loadDiquProgramesDateHourData(typename)
        }); 
	}
	...
	function loadDiquProgramesDateHourData(typename){
		var sdata = new Array(); 
		$.post("${ctx }/count/manage/learningsituation!loadDiquProgramesDateHourData.excsec?typeName="+typename+"&startTime="+Date.parse($("#startTime").val())+"&endTime="+Date.parse($("#endTime").val())+"&programesId="+$("#programesId").val(),function(data) {
	...
	function loadDiquProgrames24HourData(typename){
		var sdata = new Array(); 
		$.post("${ctx }/count/manage/learningsituation!loadDiquProgrames24HourData.excsec?typeName="+typename+"&programesId="+$("#programesId").val(),function(data) {
			var arrayObj = data.toString().split(",");　
			$(arrayObj).each(function(index){   
				sdata.push(parseInt(arrayObj[index]));  
			});
	...
	function initData(){ 
	 	$.post("${ctx }/count/manage/learningsituation!getHighchartData.excsec?programesId="+$("#programesId").val()+"&cityid="+$("#city").val()+"&tenantId="+$("#tenantId").val(),function(data) {
	...
	function createHtml(arrayValue){
		var zjOpen = false;
		...
		var html =  '<tr>'+	
					...
					'<th width="60px;">操作</th>'+
					'</tr>'; 
		var length = arrayValue.diqu.length;
		for(var i=0;i<length;i++){
			html+='<tr>';
					html+='<td>'+arrayValue.diqu[i]+'</td>';
					for(var j=0 ;j<7;j++){  
						if(j==0){
							...
							html+='<td><button class="btn btn-sm highchart-btn " type="button"   onclick="loadHighchart(6,3,\''+arrayValue.diqu[i]+'\');">24小时人数</button>&nbsp;'+
						   	  '<button class="btn btn-sm highchart-btn" type="button"   onclick="loadHighchart(7,4,\''+arrayValue.diqu[i]+'\')" >单日学习时长</button></td>';
						}
					}
			html+='</tr>'; 
		}
