...
<script>
	...
	$(document).ready(function() {
		...
		jQuery("#moveright").click(function(){
			var num = 8;
			if (($("#page option").length + $("#productPage option:selected").length) <= num){
				jQuery(this).listTolist("productPage","page","append",false);
			} else {
				b_alert("推荐列表最多"+num+"条数据！");
			}
			$("#productPage option:selected").each(function(){
				$(this).attr("selected","");
			});
		});
		...
		$("#inputForm").submit(function(){
			$("#page").find("option").each(function(){				
				$("#inputForm").append("<input type='checkbox' name='selectProductIds' checked='checked'  style='display:none' value='"+$(this).val()+"'/>");
			})
		})
		...
		jQuery("#add").click(function(){
			...
			$.ajax({
				type:"POST",
				dataType: "json",
				url: "${ctx}/related/product!exists.action",
				...
				success: function(data){
					...
					if(product==undefined){b_alert('没有查到相关书籍');return false;}
					if(Number(3)==(Number(1)+Number($("#type").val()))){
						if(product.productModel!=(Number(1)+Number($("#type").val()))&&$("#type").val()!=4)
						{

						if(isahstudyopen=='true'){
							 b_alert('添加的书籍不是<s:if test="type==2"><common:prop name="MODEL_MAJOR" propfilename="sysconfig.properties"></common:prop></s:if><s:if test="type==1"><common:prop name="MODEL_PUBMUST" propfilename="sysconfig.properties"></common:prop></s:if><s:if test="type==3">免费课</s:if>');
						}else{
							b_alert('添加的书籍不是<s:if test="type==2"><common:prop name="MODEL_MAJOR" propfilename="sysconfig.properties"></common:prop></s:if><s:if test="type==1"><s:if test="#request.isqlxx==true">校本特色课</s:if><s:else>精品课</s:else></s:if><s:if test="type==3">免费课</s:if>');
							}
							
					
					 	return false;}
					}else{
						if(product.productModel!=$("#type").val()&&$("#type").val()!=4)
						{
							if(isahstudyopen=='true'){
								b_alert('添加的书籍不是<s:if test="type==2"><common:prop name="MODEL_MAJOR" propfilename="sysconfig.properties"></common:prop></s:if><s:if test="type==1"><common:prop name="MODEL_PUBMUST" propfilename="sysconfig.properties"></common:prop></s:if><s:if test="type==3">免费课</s:if>');
							}else{
						...
						if (flag) {
							$("#page").append("<option value='"+product.id+"'>"+product.name+"</option>");
						} else {
							b_alert("此书籍已经在推荐列表中，无需重复添加！");
						}
				}
			});
		});
		
		//$('#name').autocomplete("${ctx}/product/manage/productajax.excsec", {
		//	minChars: 2,
		//	autoFill: false,
		//	scrollHeight: 220,
...
<body>
  ...
  <div class="dr-wrapper">
     <!--adminLeft结束-->
	   ...
	   <jsp:param value="productrecommend1" name="menu" />
...
<form id="inputForm" action="productrecommend!save.action" method="post">
...
<table width="98%" border="0" align="center" cellspacing="0"  class="table table-bordered dr-table-bordered">
	<tr>
		...
		<strong>已推荐的精品书籍</strong>
			<s:select list="page.result" listKey="productId" listValue="productName"
			...
			<div>
			   <p class="help-block col-md-offset-0"><s:if test="type==2">添加新书籍（书籍属性必须是<common:prop name="MODEL_MAJOR" propfilename="sysconfig.properties"></common:prop>）</s:if><s:if test="type==1">添加新书籍（书籍属性必须是<common:prop name="MODEL_PUBMUST" propfilename="sysconfig.properties"></common:prop>）</s:if>
			   </p>
			</div>
			...
			<td width="80" height="40px;" style="border:none">
			  <input type="radio" name="productgroup"  checked="checked" value="1" />书籍名称
			</td>
			...
			<tr>
			<td width="80" height="40px;" style="border:none"><input type="radio" name="productgroup"
				value="2" / ></input>书籍编码
			</td>
			<td style="border:none">
		...
		<td width="54%" valign="top">
			<strong>创建时间最新的前20门精品书籍
			 <s:select list="productPage.result" listKey="productDTO.id" listValue="productDTO.name" name="productPage1" id="productPage" cssClass="txtwb0" theme="simple" cssStyle="height:425px"
			multiple="true"/>
			</strong>
		</td>
	</tr>
