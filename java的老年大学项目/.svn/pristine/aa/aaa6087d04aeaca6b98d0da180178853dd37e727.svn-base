...
function ajaxDelete(id){
	 b_confirm('您确定要进行此操作吗?', function() {
	  ...
	  $("#tutorIds").val(tutorIds);
	   $('#coursetutor_container').load('${ahstudyUrl}/course/manage/tutor!list.excsec?tutorUserIds='+tutorIds, false);
	 });
...
<center>
	...
	function ajaxAddTutors(tutorUserIds){
		...
		$.ajax({
	         type: "POST",
	         url: "${ctx}/clazz/manage/clazz!selectClazzManager.excsec",
	         data: {
	         ...
	         success: function(data) {
			 	var data = eval(data);
			 	if(data.value=="true"){
					$('#coursetutor_container').load('${ahstudyUrl}/clazz/manage/clazz!selectClazzManager.excsec', false);
			 	}else{
	...
	<iframe src="${ctx}/clazz/manage/clazz!selectClazzManager.excsec" frameborder="0" scrolling="no" width="680" id="subMainFrame" name="subMainFrame"></iframe>
    </div>
</center>