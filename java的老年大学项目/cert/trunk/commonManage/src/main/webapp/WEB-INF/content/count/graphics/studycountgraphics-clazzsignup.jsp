...
<div class="bs-example" > 
	<ul class="nav nav-tabs nav-justified" id="tab">
        <li class="active"><a style="cursor: pointer;" href="${ctx }/count/graphics/studycountgraphics!clazzsignup.action">班级报名统计</a></li>
...
<div class="form-group">
	  	<label>数据说明：学习率=项目班级学习人数/项目班级报名人数	</label>
	</div>
</div>       
...
function initData(){  
	...
	$.post("${ctx }/count/graphics/studycountgraphics!getsignupcountdata.excsec?parameter=0&programesId="+$("#programesId").val(),function(data) {
...
function initxuexiData(){  
	var arrayValue = new Array(); 
	$.post("${ctx }/count/graphics/studycountgraphics!getsignupcountdata.excsec?parameter=1&programesId="+$("#programesId").val(),function(data) {
		var arrayObj = data.toString().split(",");　
		$(arrayObj).each(function(index){ 
			arrayValue.push(parseInt(arrayObj[index]));
		});
