...
<div class="dr-wrapper">
	...
	<jsp:param value="teamOrderCheck" name="menu" />
	...
	<!--breadcrumb-->
		...
		<li class="active"><span>集体订单审核</span></li>
		</ol>
	<!--/breadcrumb-->
	<!--页面标题-->
        <div class="dr-page-header">
            <h3>集体订单审核<small>&nbsp;</small></h3>
        </div>
        <hr></hr>
    <!--页面标题end-->
    ...
    <ul class="nav nav-tabs nav-justified">
    	<s:if test="CurTenantID==null">
	 	<li class="<s:if test='isOther!="other"'>active</s:if><s:else></s:else>" ><a href="order!teamOrderCheck.action">大平台订单</a></li>
	 	</s:if>
	    <li class="<s:if test='isOther=="other"'>active</s:if><s:else></s:else>"><a href="order!teamOrderCheck.action?isOther=other">租户订单</a></li>
	</ul>
    ...
    <!--搜索模块-->
		<div class="dr-searchbar">
			<form class="form-inline dr-form-inline" id="mainForm" role="form" name="mainForm" action="order!teamOrderCheck.action" method="post">
			...
			<div class="form-group">
			<label>审核状态</label>
			<s:select list="#{'已审核':2,'全部':0}" listKey="value" theme="simple"  listValue="key" headerValue="待审核" 	headerKey="1" value="checked" name="checked" cssClass="form-control input-sm" >
			</s:select>
			</div>
			<div class="form-group">
    	...
    	<div class="panel panel-default">
			...
			<table class="table table-bordered dr-table-bordered">
				...
				<tr>
					...
					<th width="8%" class="tt_line">支付状态</th>
					<th width="8%" class="tt_line">审核状态</th>
					<th width="15%" class="tt_line">操作</th>
				</tr>
				</thead>
				<tbody>
				<s:iterator value="page.result" status="stat">
					<tr>
						...
						<td class="white_bg">
							<s:if test="status==3&&payType==2">已审核</s:if>
							<s:elseif test="status!=3&&remark!=null"><span style="color:red;">待审核</span></s:elseif>
						</td>
						<td class="white_bg">
							<s:if test="status!=3&&remark!=null"><a href="order!info.action?id=${id }" class="btn btn-primary btn-sm" target="_blank">审核</a></s:if>
							<a href="${ctx }/clazz/manage/clazz!mmanager.action?id=${clazzId }" class="btn btn-primary btn-sm" target="_blank">设置班主任</a>
						</td>
					</tr>
				</s:iterator>
				</tbody>
