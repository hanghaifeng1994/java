...
(function($) {
    ...
    $.fn.treeview = function(settings) {
        var dfop =
            {
                ...
                url: false, //异步请求的url
                cbiconpath: "../../js/mytreeview/images/icons/", //checkbox icon的目录位置
                icons: ["checkbox_0.gif", "checkbox_1.gif", "checkbox_2.gif"],
				emptyiconpath:"../../js/mytreeview/images/s.gif", //checkbxo三态的图片
        ...
        function buildnode(nd, ht, deep, path, isend) {
			...
			if(nd.checkstate && !(dfop.showcheck && nd.showcheck))
	          cs.push("bbit-tree-selected");
            if (nd.hasChildren) {
            ...
            // 添加<a/>标识的href属性 并且当无showcheckbox是可以有选中背景
            // ======= 修改开始 ===========
            if (nd.href) {
                ...
                else if ($(et).hasClass("bbit-tree-node-cb")) // 点击了Checkbox
                {
                    ...
                    if (r != false) {
                        if (dfop.cascadecheck) {
							if(dfop.iscascade){ //是否选中节点后向下遍历
								//遍历
								cascade(check, item, s);
							}else{
								check(item, s, 1);
							}
							if(dfop.isbubble){//: true, //是否选中节点后向上上溯
		                        //上溯
	                            bubble(check, item, s);
							}
                        }
        ...
        function getck(items, c, fn) {
            for (var i = 0, l = items.length; i < l; i++) {
            	...
            	//if(items[i].showcheck == true && items[i].checkstate == 1)
            	//	c.push(fn(items[i]));
                //alert("tck2 q"+c);
    ...
    $.fn.getTSVs = function() {
    	//alert("Tsvs");
        if (this[0].t) {
        	//alert("Tsvs"+this[0].t.getSelectedValues());
