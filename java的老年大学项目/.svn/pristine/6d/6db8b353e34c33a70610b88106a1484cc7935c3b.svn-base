 ...
 */
@Service
public class NewsArticleWyService extends WeyeBaseService<NewsArticle> {
    ...
    public void  publish(NewsArticle na){
        ...
        List<PtsetSiteVo> siteIds= newsUtil.GetPriSite(siteId);
       //查询出该站点所有的子站点及本身，长度大于1 表示为主站点，否则为子站点
        try{
                for (PtsetSiteVo pt:siteIds) {
                    NewsNewsSiteRel nnsr=new NewsNewsSiteRel();
    ...
    public boolean  cancelPublish(NewsArticle na)  {
        try{
            ...
            NewsUtil newsUtil=new NewsUtil();
            List<PtsetSiteVo> siteIds= newsUtil.GetPriSite(na.getSiteId());
            for (PtsetSiteVo pt :siteIds){
