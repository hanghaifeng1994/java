...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$('#selSchema').attr("checked",'true');//
			$('#selSchema').change(function(){
				var isDef = $(this).is(':checked') ;
				var url = '${ctx}/gen/genTable/getTables?defSchema=' + isDef;
				$.getJSON(url, function(list){
					var ss = '';
					for(var i=0; i<list.length; i++){
						ss += '<option value="' + list[i].name +
								'">' + list[i].name +
								'</option>';
					}
					$('#tableName').empty();
	...
	<c:choose>
		<c:when test="${empty genTable.name}">
			<form:form id="inputForm" modelAttribute="genTable" action="${ctx}/gen/genTable/form" method="post" class="form-horizontal">
				...
				<div class="control-group">
					<label class="control-label">配置表:</label>
					<div class="controls">
						<input type="checkbox" name="selSchema" id="selSchema">
					</div>
				</div>
				<div class="control-group">
					<label class="control-label">表名:</label>
					<div class="controls">
						<form:select path="name" id="tableName" class="input-xxlarge">
		...
		<c:otherwise>
			<form:form id="inputForm" modelAttribute="genTable" action="${ctx}/gen/genTable/save" method="post" class="form-horizontal">
				<form:hidden path="id"/>
				<form:hidden path="dbSchema"></form:hidden>
				<sys:message content="${message}"/>
				<fieldset>
					<legend>基本信息</legend>
					<div class="control-group">
