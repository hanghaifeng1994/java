...
<div class="bs-example" > 
	<ul class="nav nav-tabs nav-justified" id="tab">
        ...
        <li  class="active"><a style="cursor: pointer;" href="${ctx }/count/graphics/studycountgraphics!areaavgstudytime.action">地区平均学习时间</a></li>
...
<div class="form-group"  <s:if test="filter==2">style="display: none;"</s:if> >
	...
	<div class="form-group">
	  	<label>数据说明：地区平均学习时间 = 地区的总学习时间/地区报名人数</label>
	</div>
</div>       
...
function loadxAxis(){	
	...
	if(cityId==''||cityId=='undefined'){
			$($("#cityId option")).each(function(index, domEle){
				if(domEle.text!='--所属城市--'){
					res.push(domEle.text); 
				}
			});
	}else{ 
		if(cityId) {
			$.post("${ctx}/count/graphics/studycountgraphics!getAreaJsonByFilter.excsec",{"cityId":cityId,"areaId":'${areaId}',"filter":'<s:property value="filter" />'},function(data) {
...
function initData(){  
	...
	$.post("${ctx }/count/graphics/studycountgraphics!getareaavgstudycountdata.excsec?areaId=${areaId}&cityId="+$("#cityId").val()+"&tenantId=${currentTenant.id}"+"&filter=<s:property value="filter" />",function(data) {
