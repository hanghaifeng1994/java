	 ...
	 //验证批量删除文章的列表非空与否
	   $("#batchDelDown").click(
			   function(){
				   ...
				   document.getElementById("deleteForm").action="${ctx }/data/manage/canton!batchDeleteed.action?parentId=${parentId}";
				       if(!checkSelect()) {
			            b_alert("没有可操作记录,请勾选");
			            return false;
			       } 
...
<form id="mainForm" name="mainForm" action="canton.action"
...
<form name="deleteForm" id="deleteForm" action="canton.action" method="post">
<div class="panel panel-default">
    <div class="btn-toolbar dr-btn-toolbar">
        ...
        <button class="btn btn-primary btn-sm" type="button" id="batchDelDown" name="Submit3" onclick="window.location='canton!input.action?parentId=${parentId}'">
        <span class="glyphicon glyphicon-plus"></span>
                    新增
        </button>
	    
	<s:if test="parentId != null">
	   <button class="btn btn-primary btn-sm" type="button" id="batchDelDown" name="Submit3" onclick="window.location.href='canton!reback.action?parentId=${parentId}'">
		...
		<tr>
			...
			<s:else>
				...
				<td title="${code}" ><common:cut len="20"
					string="${code}" />&nbsp;</td>
			<td>
			<a href="${ctx }/data/manage/canton!input.action?id=${id}&parentId=${parentId}" class="btn btn-primary btn-sm" type="button">
			<span class="glyphicon glyphicon-edit"></span>
			编辑</a>
			
			<s:if test="parentId != '' && parentId != null">
				<button class="btn btn-default btn-sm" type="button" onclick="delRecord('${ctx }/data/manage/canton!delete.action?id=${id}&parentId=${parentId}');">
               <span class="glyphicon glyphicon-trash"></span>
                                  删除
               </button>	
			</s:if> 
			<a href="${ctx }/data/manage/canton.action?parentId=${id}" class="btn btn-primary btn-sm" type="button">
...
<script>
		...
		function folderOpen(parentId){
			...
			$.ajax({
		         type: "POST",
		         url: "${ctx}/data/manage/canton!folderOpen.action",
		         data: {
		       	 	"parentId":parentId
				 },
