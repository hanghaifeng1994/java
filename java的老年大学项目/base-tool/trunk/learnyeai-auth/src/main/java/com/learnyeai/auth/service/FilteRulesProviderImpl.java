...
public class FilteRulesProviderImpl implements ShiroFilteRulesProvider{
	...
	public List<RolePermRule> loadRolePermRules(){
		...
		Map<String, String> list = baseClient.queryAllUrl2Perms();
		List<RolePermRule> rstList = new ArrayList<>();
		list.forEach((key,val)->{
			RolePermRule rolePermRule = new RolePermRule();
			rolePermRule.setUrl(key);
			rolePermRule.setNeedPerms(val);
			rstList.add(rolePermRule);
		});
		return rstList;
	}
