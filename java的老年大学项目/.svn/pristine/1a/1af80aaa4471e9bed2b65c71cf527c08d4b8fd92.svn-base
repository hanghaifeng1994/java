...
<head>
<title>员工管理-<s:if test="currentTenant!=null">${currentTenant.logo}</s:if><s:else><%@ include
				...
				<s:if test="nowUser.tenantId==null">
					...
					<ul class="nav nav-tabs nav-justified">
						<li
							...
							href="manager.action">大平台员工 </a></li>
						<li class="<s:if test="tabFlag2=='other'">active</s:if>"><a
							href="manager.action?tabFlag2=other">租户员工</a></li>
					</ul>
				</s:if>
						...
						<div class="dr-searchbar">
							<s:if test="tabFlag2=='other'">
								<div class="form-group">
									<label>租户</label>
									<s:select list="tenantLists" listKey="id" value="tenantId"
										listValue="name" theme="simple" cssClass="form-control"
										name="tenantId" headerValue="--选择租户--" headerKey=""></s:select>
								</div>
							</s:if>
						
								<div class="form-group">
								<label>姓名</label> <input class="form-control input-sm"
									type="text" id="sname" value="${name}">
							</div>
							<div class="form-group">
								<label>手机号</label> <input class="form-control input-sm"
									type="text" id="smobile" value="${realname}">
							</div>

							<div class="form-group">
								<button class="btn btn-default btn-sm" type="button" onclick="init()">
									<span class="glyphicon glyphicon-search" id="btnsearch"></span> 搜索
								</button>
							</div>
						</div>
						...
						<div class="panel panel-default">
							<div class="btn-toolbar dr-btn-toolbar">
								<span class="fl">
									...
									<button class="btn btn-primary btn-sm" type="button"
										name="Submit3"
										onclick="window.location='user!input.action'">
										<span class="glyphicon glyphicon-plus"></span> 新增员工									</button>
								</span>
							</div>
...
<tr>
											...
											<td>&nbsp;{{status_str}}</td>
											<td><a
												href="${ctx}/user/manage/user!modify.action?id={{shopuser_id}}"
								...
								<tbody id="list">
									...
									<s:iterator value="page.result" status="stat">
										<tr>
											...
											<td><a
												href="${ctx}/user/manage/manager!modify.action?id=${shopuser_id}"
												class="btn btn-primary btn-sm"> <span
													class="glyphicon glyphicon-edit"></span>&nbsp;编辑
											</a> </td>
...
function init(){
	params.payload.username=$("#susername").val();
	params.payload.name=$("#sname").val();
	params.payload.mobile=$("#smobile").val();
	params.payload.role="user";
	api.loadpage('${ctx}','SellApiWeb/api/manage/shopuser/userList.do','listTemplate','list',init)
}
...
$(function(){
	...
	$("#batchDelDown").click(function (){
		   ...
		   b_confirm('您确定要进行此操作吗?', function() {
			   params.payload.shopuser_id=selectids;
				api.exec('${ctx}',"SellApiWeb/api/manage/shopuser/del.do",init)
  	   		});
	})
});
</script>
