    ...
    function exportexcel(){
     ...
     function changecity(){				
		var cityId = $("#cityId").val();
			$.post("${ctx}/count/manage/studentcountclazz!getAreaJson.excsec",{"cityId":cityId},function(data) {
     ...
     function changecanton(){				
  		...
  		var cityId = $("#cityId").val();
  			$.post("${ctx}/count/manage/studentcountclazz!getClazzBycanton.excsec",{"cantonId":cantonId,"cityId":cityId},function(data) {
  				$("#clazzId option[value!='']").remove();
  				var practiceUnit = eval(data);
  				if(practiceUnit.length == 0) $("#clazzId option[value!='']").remove();
  				for(var i = 0 ;i < practiceUnit.length ; i++) {
  					if(i==0){
     ...
     function changeTenant(){				
 		var tenantId = $("#tenantId").val();
 			$.post("${ctx}/count/manage/studentcountclazz!getClazzs.excsec",{"tenantId":tenantId},function(data) {
 				$("#clazzId option[value!='']").remove();
 				var practiceUnit = eval(data);
 				if(practiceUnit.length == 0) $("#clazzId option[value!='']").remove();
 				for(var i = 0 ;i < practiceUnit.length ; i++) {
...
<!--正文左边开始-->
	...
	<jsp:param value="studentcountclazz" name="menu" />
...
<!--信息提示 end-->
<form class="form-inline dr-form-inline" id="mainForm" name="mainForm" action="${ctx}/count/manage/studentcountclazz.action" method="post">
...
<div class="form-group">
	  	<label>所属城市</label>
	  	<s:select list="tenantCityLists" onchange="changecity()" listKey="id" listValue="name" id="cityId" name="cityId" value="cityId" theme="simple" headerValue="--所属城市--" headerKey="" cssClass="form-control" ></s:select>
</div>

<div class="form-group">
	  	<label>所属区域</label>
	  	<s:select cssClass="form-control" list="areaVLists" onchange="changecanton()"  listKey="id" listValue="name" id="cantonId" name="cantonId" theme="simple" value="cantonId" headerKey="" headerValue="--所属区域--" />
</div>
<div class="form-group">
	  	<label>所属班级</label>
	  	<s:select list="clazzLists" listKey="id" value="clazzId" listValue="name" theme="simple"  cssClass="form-control" name="clazzId" headerValue="--所属班级--" headerKey=""></s:select>
</div>
 	...
 	<div class="btn-toolbar dr-btn-toolbar">
		       <button name="Submit2" type="button" class="btn btn-default btn-sm" id="exportbut" 
			    ...
			    <label class="ml10" id="downLoad" style="display: none">
					<a href="" target="_blank">班级个人学情导出文件下载</a>
				</label>
			    </span>
