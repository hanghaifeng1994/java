...
public class GenTableService extends BaseService {
	...
	public List<GenTable> findTableListFormDb(GenTable genTable){
		...
		// 查询两数据库的表
		if(genTable.getDbSchema() == null || genTable.getDbSchema().length() == 0){
			...
			list.addAll(genDataBaseDictDao.findTableList(genTable));
		}else{
			list = genDataBaseDictDao.findTableList(genTable);
		}
		return list;
	...
	public GenTable getTableFormDb(GenTable genTable){
		...
		if (StringUtils.isNotBlank(genTable.getName())){
			...
			List<GenTable> list = this.findTableListFormDb(genTable);
			if (list.size() > 0){
