 ...
 */
public class ClientThreadStart implements ILearnAiThreadStart {

    private ClientBaseInfoGet clientBaseInfo;

    public ClientThreadStart(ClientBaseInfoGet clientBaseInfo) {
    ...
    public void initUserSession(IBusinessContext ctx) {
        ...
        CustInfoVo custInfo = clientBaseInfo.queryCustInfo(accountId);
        if(null == custInfo){
            AresRuntimeException e = new AresRuntimeException(ReportErrorKey.RTN_TIME_OUT);
        }
    ...
    public void initThreadContex(IBusinessContext ctx) {
        ...
        PtsetSiteVo site = clientBaseInfo.querySite(curSiteId);
