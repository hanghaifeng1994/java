...
//					ActUtils.exportDiagramToFile(repositoryService, processDefinition, exportDir);
	...
	@Transactional(readOnly = false)
	public void deleteDeploymentByFlowKey(String flowKey) {
		List<ProcessDefinition> lists = repositoryService.createProcessDefinitionQuery().processDefinitionKey(flowKey) // 使用key查询
				.list();
		// 遍历,获取流程定义的id
