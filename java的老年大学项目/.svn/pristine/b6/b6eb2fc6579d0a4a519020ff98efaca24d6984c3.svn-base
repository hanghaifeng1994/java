...
import com.learnyeai.course.api.service.WkHomeworkApiService;
...
public class CrsCourseHomeworkWyService extends WeyeBaseService<CrsCourseHomework> {
	...
	@Resource
	private WkHomeworkApiService wkHomeworkApiService;
	...
	public List<CrsCourseHomework> homeworkList(CrsCourseHomework t) {
		...
		for (CrsCourseHomework ch : list) {
			WkHomeworkVo homework = wkHomeworkApiService.queryHomeworkApi(ch.getHwId());
			if (homework != null) {
				ch.setHwTitle(homework.getHwTitle());
			}
		}
		return list;
	}
