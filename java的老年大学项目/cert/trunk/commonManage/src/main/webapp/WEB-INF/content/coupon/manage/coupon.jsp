...
<html>
<head>
<title>优惠规则管理-<s:if test="currentTenant!=null">${currentTenant.logo}</s:if><s:else><%@ include
				...
				<s:if test="nowUser.tenantId==null">
					...
					<ul class="nav nav-tabs nav-justified">
						<li
							...
							href="manager.action">大平台优惠规则 </a></li>
						<li class="<s:if test="tabFlag2=='other'">active</s:if>"><a
							href="manager.action?tabFlag2=other">租户优惠规则</a></li>
					</ul>
				</s:if>
						...
						<div class="panel panel-default">
							<div class="btn-toolbar dr-btn-toolbar">
								<span class="fl">
									...
									<button class="btn btn-primary btn-sm" type="button"
										name="Submit3"
										onclick="window.location='coupon!input.action'">
										<span class="glyphicon glyphicon-plus"></span> 新增优惠规则
									</button>
								</span>
							</div>
...
<tr>
											...
											<td>&nbsp;{{status_str}}</td>
											<td><a
												href="${ctx}/coupon/manage/coupon!modify.action?id={{coupon_id}}"
												class="btn btn-primary btn-sm"> <span
													class="glyphicon glyphicon-edit"></span>&nbsp;编辑
											</a>


							<a	href="${ctx}/coupon/manage/coupon!detaillist.action?id={{coupon_id}}"
												class="btn btn-primary btn-sm"> <span
													class="glyphicon glyphicon-edit"></span>&nbsp;规则明细</a>
 </td>
										</tr>
				{{/each}}
			</script>
...
function init(){
	api.loadpage('${ctx}','SellApiWeb/api/manage/coupon/couponList.do','listTemplate','list',init)
}
...
$(function(){
	...
	$("#batchDelDown").click(function (){
		...
		params.payload.coupon_id=selectids;
		api.exec('${ctx}',"SellApiWeb/api/manage/coupon/del.do",init)
		   });
	})
});
</script>
