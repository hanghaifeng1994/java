...
<title>编辑员工信息-<s:if test="currentTenant!=null">${currentTenant.logo}</s:if><s:else><%@ include file="/common/title.jsp" %></s:else></title>
...
<!--adminLeft结束-->
  ...
  <div class="dr-container-fluid">
    <ol class="dr-breadcrumb">
     ...
     <li><a href="#">员工管理</a></li>
     <li class="active">
        <span class="deep_bule">编辑普通员工 </span>
     </li>
    </ol>
...
<form id="inputForm" name="inputForm" class="form-horizontal dr-form-bordered" 
  ...
  <div class="dr-form-title clearfix">
    <div class="col-md-12">
    <h4 class="text-primary">更改员工基本信息</h4>
    </div>
   </div>
   <div id="detail">
   ...
   <div class="form-group" id="validate_username">
	 <label class="col-md-2 control-label">手机号<span class="text-danger">*</span></label>
        <div class="col-md-3">
	    <input id="username" name="username" type="text" value="{{data.username}}" class="form-control" readonly="readonly"/> 
       </div>
   ...
   <div class="form-group" id="validate_name">
     ...
     <div class="form-group" id="validate_name" style="display:None;">
	    <label class="col-md-2 control-label">手机号<span class="text-danger">*</span></label>
	    <div class="col-md-3">
	       <input id="mobile" name="mobile" value="{{data.mobile}}" type="text" class="form-control"/>
        </div>
        <span class="help-block" id="error_name"></span>
     </div>
   ...
   <div class="form-group" id="validate_name">
	    <label class="col-md-2 control-label">品牌<span class="text-danger">*</span></label>
	    <div class="col-md-3">
...
$(function(){
	...
	params.payload.shopuser_id=api.p('id');
	api.load('${ctx}','SellApiWeb/api/manage/shopuser/detail.do','detailTemplate','detail',function (d){$('#brand_id').val(d.data.brand_id)})
	...
	$("#btnsave").click(function (){
		...
		api.exec('${ctx}','SellApiWeb/api/manage/shopuser/modify.do',function (){window.location.href='user.action';})
		}
	)
});
</script>
