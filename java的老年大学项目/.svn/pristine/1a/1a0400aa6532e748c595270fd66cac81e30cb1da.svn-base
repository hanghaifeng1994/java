...
<html>
<head>
<title>书籍分类管理<s:if test="currentTenant!=null">-${currentTenant.logo}</s:if><s:else><%@ include file="/common/title.jsp" %></s:else></title>
...
<script type="text/javascript">
    $(document).ready(function(){
	 ...
	 //验证批量删除文章的列表非空与否
	   $("#batchDelDown").click(function(){
				   ...
				   document.getElementById("deleteForm").action="${ctx }/product/manage/productcategory!delete.action?parentId=${parentId}";
				       if(!checkSelect()) {
			            b_alert("没有可操作记录,请勾选");
			            return false;
			       } 
	   ...
	   $("#hide").click(
			   ...
			   function(){
				   ...
				   var oaction = document.getElementById("deleteForm").action;
				   document.getElementById("deleteForm").action="${ctx }/product/manage/productcategory!hide.action?parentId=${parentId}";
			       if(!checkSelect()) {
			            b_alert("没有可操作记录,请勾选");
			            return false;
			       } 
		...
		$("#show").click(
				   function(){
					   var oaction = document.getElementById("deleteForm").action;					   
					   document.getElementById("deleteForm").action="${ctx }/product/manage/productcategory!show.action?parentId=${parentId}";
					   if(!checkSelect()) {
				            b_alert("没有可操作记录,请勾选");
				            return false;
				       } 
    ...
    function deleted(id){
 		var oaction = document.getElementById("deleteForm").action;
 		document.getElementById("deleteForm").action="${ctx }/product/manage/productcategory!delete.action?ids="+id;
	       b_confirm('您确定要进行此操作吗?', function() {
				$("#deleteForm").submit();
				document.getElementById("deleteForm").action = oaction;
	   	});
...
<!--正文左边开始-->
	...
	<jsp:param value="productcategory" name="menu" />
...
<s:if test="nowUser.tenantId!=null">
     <ul class="nav nav-tabs nav-justified">
	     <li class="<s:if test='isMy'>active</s:if><s:else></s:else>" ><a href="productcategory.action?isMy=true">我的书籍分类</a></li>
         <li class="<s:if test='!isMy'>active</s:if><s:else></s:else>"><a href="productcategory.action?isMy=false">公共书籍分类</a></li>
    </ul>
</s:if>
<s:elseif test="nowUser.tenantId==null">
<ul class="nav nav-tabs nav-justified">
	     <li class="<s:if test='isMy'>active</s:if><s:else></s:else>" ><a href="productcategory.action?isMy=true">我的书籍分类</a></li>
         <li class="<s:if test='!isMy'>active</s:if><s:else></s:else>"><a href="productcategory.action?isMy=false">其他租户书籍分类</a></li>
    </ul>
</s:elseif>
<div class="tab-content dr-tabs-panel">
<form class="form-inline dr-form-inline" id="mainForm" name="mainForm" action="productcategory.action" method="post">
<input type="hidden" name="page.pageNo" id="pageNo" value="${page.pageNo}" />
<input type="hidden" name="page.orderBy" id="orderBy" value="${page.orderBy}" />
<input type="hidden" name="page.order" id="order" value="${page.order}" />
<input type="hidden" name="isMy" id="isMy" value="${isMy}" />
</form>
<form name="deleteForm" id="deleteForm" action="productcategory.action" method="post">
<input type="hidden" name="isMy" id="isMy" value="${isMy}" />
...
<div class="btn-group">
<button class="btn btn-primary btn-sm" type="button" name="Submit3" onclick="window.location='productcategory!input.action?parentId=${parentId}&isMy=${isMy}'">
...
<div class="btn-group">
<button class="btn btn-primary btn-sm" type="button" name="Submit3" onclick="window.location='productcategory!reback.action?parentId=${parentId}&isMy=${isMy}'">
...
<table class="table table-bordered dr-table-default">
	...
	<s:iterator value="page.result" status="stat">
		...
		<s:if test="isMy && nowUser.tenantId==null || nowUser.tenantId!=null">
			...
			<td class="white_bg">
			<s:if test="isMy && (parentId != '' && parentId != null || nowUser.tenantId==null|| nowUser.tenantId!=null)">			 			 		 
			<a class="btn btn-primary btn-sm" href="${ctx }/product/manage/productcategory!moveUp.action?parentId=${parentId}&id=${id }&sortnum=${orderNum}&tenantId=${tenantId}&isMy=${isMy}">
		       <span class="glyphicon glyphicon-arrow-up"></span>&nbsp;上移
		    </a>	
			<a class="btn btn-default btn-sm"  href="${ctx }/product/manage/productcategory!moveDown.action?parentId=${parentId}&id=${id }&sortnum=${orderNum}&tenantId=${tenantId}&isMy=${isMy}">
			   <span class="glyphicon glyphicon-arrow-down"></span>&nbsp;下移
		    </a>
			<button class="btn btn-primary btn-sm" type="button" onclick="window.location='${ctx }/product/manage/productcategory!input.action?id=${id}&parentId=${parentId}&isMy=${isMy}'">
			...
			<button class="btn btn-primary btn-sm" type="button" onclick="window.location='${ctx }/product/manage/productcategory.action?parentId=${id}&isMy=${isMy}'">
...
<script>
		jQuery(document).ready(function () {
			//TreeView.init();
			<s:iterator value="productCategories" status="stat">
			var htm = "";
		...
		function folderOpen(parentId){
			...
			$.ajax({
		         type: "POST",
		         url: "${ctx}/product/manage/productcategory!folderOpen.action",
		         data: {
		       	 	"parentId":parentId
				 },
		...
		function selectTree(id){
			window.location.href = "productcategory!selectTree.action?id="+id;
		}
