...
public class ResFileWyService extends WeyeBaseService<ResFile> {
	...
	@Override
	protected Weekend<ResFile> genSqlExample(ResFile t, Map params) {
		Weekend<ResFile> w = super.genSqlExample(t, params);
		WeekendCriteria<ResFile, Object> c = w.weekendCriteria();
		if (StringUtils.isNotBlank(t.getResId())) {
			c.andEqualTo(ResFile::getResId, t.getResId());
		}
		if (StringUtils.isNotBlank(t.getFileId())) {
			c.andEqualTo(ResFile::getFileId, t.getFileId());
		}
		w.and(c);
		return w;
	}
