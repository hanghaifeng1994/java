...
<head>
	...
	function onCertplateText(certtemplateId,flag){
		if(certtemplateId){
			$.ajax({
				type:"post",    //请求方式
				url:"${certapiurl}/cert/certtemplate/manage/ajaxCertplateText",
	...
	function addCerttemplate(){
	    var  templateId = $("#templateId").val();
		$.ajax({
				type:"post",    //请求方式
				url:"${certapiurl}/cert/certtemplate/manage/all",
				dataType:"jsonp",    //跨域json请求一定是jsonp
				jsonp: "callback",    //跨域请求的参数名，默认是callback
				jsonpCallback:"successCallback",    //自定义跨域参数值，回调函数名也是一样，默认为jQuery自动生成的字符串
				data:{},    //请求参数
				success: function(data) {
				    //请求成功处理，和本地回调完全一样
				    $.each(data.certtemplateVOs,function(i,v){
				    	if(v.certemplateId==templateId){
				    		$("#certtemplateId").append("<option value=\""+v.certemplateId+"\" selected=\"selected\">"+v.certtemplateName+"</option")
				    	}else{
				    		$("#certtemplateId").append("<option value=\""+v.certemplateId+"\">"+v.certtemplateName+"</option")
				    	}
...
<li style="list-style-type:none;margin-left: 10px;">
 <%-- <s:select list="certtemplateVOs" listKey="certemplateId"  value="certtemplateId"
	...
	<select name="certtemplateId" id="certtemplateId" style="width:220px;margin-left: -48px;" class="form-control input-sm" onchange="onCertplateText($(this).val(),true)">
	</select>
</li>
</ul>
<ul id="liUl">
<s:set var="certTypeLists" value="@com.drcl.traincore.contants.CerttemplateField@enumOfflineTextList"></s:set>
...
<!-- 图片显示 -->
<s:set var="certImageLists" value="@com.drcl.traincore.contants.CerttemplateField@enumOfflineImageList"></s:set>
