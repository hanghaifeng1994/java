...
public class CryptoKeyUtil {
	 ...
	 * @param key
	 * @return
	 * @Author Lewis(lgs@yitong.com.cn) 2014-10-29 上午10:59:06
	 */
	public static StringBuffer encrypt(StringBuffer result, String input,Key key) {
		result.append(input);
		try {
			return new StringBuffer(BASE64Custom.encode(AESCoder.encrypt(result
	 ...
	 * @param key
	 * @return
	 * @Author Lewis(lgs@yitong.com.cn) 2014-10-29 上午10:59:45
	 */
	public static StringBuffer delDecrypt(StringBuffer params,Key key){
		StringBuffer result = new StringBuffer();
		try {
			String decryptStr = new String(AESCoder.decrypt(
					BASE64Custom.decode(params.toString()), key));
			//删除最后一位数据
			result.append(decryptStr.substring(0, decryptStr.length()-1));
			return result;
		} catch (Exception e) {
	 ...
	 * @param key
	 * @return
	 * @Author Lewis(lgs@yitong.com.cn) 2014-10-29 上午11:00:15
	 */
	public static StringBuffer decrypt(String params,Key key) {
		StringBuffer result = new StringBuffer();
		String end = params;
		do {
			try {
	 ...
	 * @param key
	 * @return
	 * @Author Lewis(lgs@yitong.com.cn) 2014-10-29 下午9:23:21
	 */
	public static String encryptAll(String str,Key key) {
		try {
			return BASE64Custom.encode(AESCoder.encrypt(str.getBytes(), key));
		} catch (Exception e) {
			e.printStackTrace();
		}
		return "";
	...
	public static String decryptAll(String params){
		try {
			return new String(AESCoder.decrypt(
					BASE64Custom.decode(params), k));
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
...
//		String IV = "YTXABCBANKPSKEYS"; // 原始iv
		if (AESCoder.IV.length() != 16) {
			//默认KEY
			k = AESCoder.toKey("abcdefghijklmnop".getBytes());
		} else {
