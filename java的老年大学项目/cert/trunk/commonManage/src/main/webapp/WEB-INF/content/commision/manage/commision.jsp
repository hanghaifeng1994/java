...
<html>
<head>
<title>佣金规则管理-<s:if test="currentTenant!=null">${currentTenant.logo}</s:if><s:else><%@ include
				...
				<s:if test="nowUser.tenantId==null">
					...
					<ul class="nav nav-tabs nav-justified">
						<li
							...
							href="manager.action">大平台佣金规则 </a></li>
						<li class="<s:if test="tabFlag2=='other'">active</s:if>"><a
							href="manager.action?tabFlag2=other">租户佣金规则</a></li>
					</ul>
				</s:if>
						...
						<div class="panel panel-default">
							<div class="btn-toolbar dr-btn-toolbar">
								<span class="fl">
									...
									<button class="btn btn-primary btn-sm" type="button"
										name="Submit3"
										onclick="window.location='commision!input.action'">
										<span class="glyphicon glyphicon-plus"></span> 新增佣金规则
									</button>
								</span>
							</div>
...
<tr>
											<td><input type="checkbox" name="ids"
												value="{{commision_id}}" /></td>
											<td>&nbsp;{{rate}}</td>
											<td>&nbsp;{{start_time}}</td>
											<td>&nbsp;{{end_time}}</td>
											<td>&nbsp;{{status_str}}</td>
											<td><a
												href="${ctx}/commision/manage/commision!modify.action?id={{commision_id}}"
...
function init(){
	api.loadpage('${ctx}','SellApiWeb/api/manage/commision/commisionList.do','listTemplate','list',init)
}
...
$(function(){
	...
	$("#batchDelDown").click(function (){
		...
		params.payload.commision_id=selectids;
		api.exec('${ctx}',"SellApiWeb/api/manage/commision/del.do",init)
		   });
	})
});
</script>
