...
public class CfgFunctionService extends MybatisBaseService<CfgFunction> {
    ...
    public int save(CfgFunction params) {
        ...
        CfgFunction parent = null;
        String parentIds = null;

        if(StringUtils.isNotBlank(params.getParentId())){
            parent = queryById(params.getParentId());
        }
        if(null != parent) {
            parentIds = ( parent.getParentIds() == null ? "" : parent.getParentIds()) + params.getParentId() + ",";
        }

        String oldParentIds = null;
        if(null != parent && StringUtils.isNotBlank(params.getFuncId())) {
