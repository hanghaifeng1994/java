...
public class CustInfoWyService extends WeyeBaseService<CustInfo> {
    ...
    public MyPage queryStaffPage(Map params){
        ...
        rstPage.setPageNo(page.getPageNo());
    ...
    @Transactional
    public int batchaddUser_idCar(List<CustInfo> custList){

        int total = 0;
        List<CustInfo> rstList = new ArrayList<>();
        for(CustInfo it : custList){
            CustInfo pp = new CustInfo();
            pp.setIdcarNo(it.getIdcarNo());
            List<CustInfo> list = queryList(pp, null);
            ...
            rstList.add(custInfo);
        }
        custList.clear();
        custList.addAll(rstList);
        return total;
    }
}
