...
public abstract class TreeService<D extends TreeDao<T>, T extends TreeEntity<T>> extends CrudService<D, T> {
	...
	public void save(T entity) {
		...
		// 设置新的父节点串
		String pids = entity.getParent().getParentIds()+entity.getParent().getId()+",";
		entity.setParentIds(pids);

		// 机构等级
		entity.setGrade(pids.split(",").length);
		...
		for (T e : list){
			if (e.getParentIds() != null && oldParentIds != null){
				...
				e.setGrade(e.getParentIds().split(",").length);
