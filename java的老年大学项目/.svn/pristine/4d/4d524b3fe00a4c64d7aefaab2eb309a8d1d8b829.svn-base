...
<form id="mainForm" name="mainForm" action="refund-record.action" method="post">
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr>
		...
		<td width="15%" class="tt_line">退单人</td>
		<td width="15%" class="tt_line">退单时间</td>
		<td width="15%" class="tt_line">退单结果</td>
	</tr>
	<s:iterator value="page.result" status="stat">
		<tr>
			<td class="white_bg"><a href="order!info.action?id=${orderform.id }"  target="_blank" >${orderform.sn}</a></td>
			<td class="white_bg">${refundOperator}</td>
			<td class="white_bg">${refundTime}</td>
			<td class="white_bg">${refundresultStr}</td>
		</tr>
	</s:iterator>
</table>
</div>
