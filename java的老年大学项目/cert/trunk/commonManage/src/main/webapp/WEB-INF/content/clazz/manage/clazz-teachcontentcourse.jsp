...
function addCourseToUser(){
	if(checksel()){
		if(!confirm('您确定要将此课程增加到现有学员吗?')) return false;
		$("#addtostudent").val("1");
		$("#dataForm").submit();
	}
}
...
<!--信息提示 end-->
   ...
   <form id="mainForm" action="clazz!teachcontentcourse.action" method="post" class="form-inline dr-form-inline">
    ...
    <div class="form-group">
      <label>课程名称:</label>
      <input name="filter_LIKES_name" value="${param['filter_LIKES_course.name']}"  class="form-control input-sm" style="width: 200px;"/>
    </div>
    <div>
    <div class="form-group">
<label>教学方式</label>
...
<form id="dataForm"	action="clazz!saveteachcontentcourse.action"	method="post">
  <input type="hidden" name="teachContentId" value="${teachContentId}" />
  <input type="hidden" name="id" value="${id}" />
  <input type="hidden" name="programId" value="${programId}" />
  <input type="hidden" name="addtostudent" id="addtostudent" value="0" />
   ...
   <thead>
	...
	<s:iterator value="courseVOs" status="stat">
		<tr>
			...
			<td><common:cut len="30" string="${name}" /></td>
			<td>${serialNo}</td>
			<td>${studylength}</td>
...
<div class="dr-panel-footer">
  ...
  <div class="text-center">
        <button class="btn btn-primary" type="button" name="Submit32" onclick="addCourseToUser();">
