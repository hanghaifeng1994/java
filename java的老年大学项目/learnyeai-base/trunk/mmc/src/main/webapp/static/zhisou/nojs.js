...
(function (window, $) {
    ...
    $nojs.fileupload = {
        ...
        init: function (context) {
            ...
            $(this.selector, context).each(function (i, fileInput) {
                ...
                if (needPreview === true) {
                    //var $preview = $('<img class="preview" style="width: 10em; margin:0px 0.5em 0.5em;"/>');
                    var previewStyle = $fileInput.data("previewStyle");//获取图片预览样式
                    var $preview = $('<img class="preview"/>');
                    if(!previewStyle)
                        previewStyle = 'single_img';
                    $preview.addClass(previewStyle);

                    $fileInput.after($preview);
                    $fileInput.data("nojs.fileupload.$preview", $preview);
                    if ($displayId.val()) {
                        $preview.attr("src", resourcePath + $displayId.val());
    ...
    $nojs.fileuploadMultiple = {
        ...
        options: {
            ...
            showPreview: true,
            schema: "string",
            previewWidth:false, // width，自定义宽度
            previewHeight: false, //height，自定义高度
            isFile: false,// 如果是文件，使用文件模板
            fileNames: ''// 是文件，文件链接显示文件名字，文件名字用逗号分割
        },
        ...
        $template: {
            ...
            $previewFileItem: $('<div class="clearfix preview-item"><button type="button" class="close">&times;</button><a target="_blank" class=""></a></div>')
        },
        ...
        preview: function ($preview, options) {
            ...
            this.addImage = function (data) {
                ...
                // 如果是文件
                if(options.isFile){
                    $previewItem = $nojs.fileuploadMultiple.$template.$previewFileItem.clone();
                    var $a = $previewItem.find("a");
                    $a.attr("href", this.getPreviewUrl(value));
                    if(!fileName)
                        fileName = '附件';
                    $a.html(fileName);
                }else {
                    $previewItem = $nojs.fileuploadMultiple.$template.$previewItem.clone();
                    var $img = $previewItem.find("img");
                    $img.attr("src", this.getPreviewUrl(value));
                ...
                $previewItem.data("value", value);
                $previewItem.attr("id", id);
                $previewItem.find("button.close").click(function () {
                    preview.options.$element.trigger("removefile", {id:id,value:value});
                });
            ...
            string: {
                init: function ($value, preview) {
                    ...
                    if (originValue) {
                        ...
                        var names = null;
                        if(options.isFile && options.fileNames){
                            names = options.fileNames.split(",");
                        ...
                        for (var i = 0; i < values.length; i++) {
                            if(!values[i] || values[i].length == 0)
                                continue;
                            preview.addImage({value:values[i], fileName: null==names?null : names[i]});
                        }
                    }
                },
                addValue: function ($value, data, preview) {
                    var value = data.value;
                    var originValue = $value.val();
                    if (originValue) {
                    ...
                    preview.addImage(data);
                },
                removeValue: function ($value, data, preview) {
        ...
        init: function (context) {
            ...
            $(this.selector, context).each(function (i, fileInput) {
                ...
                $fileInput.on("addfile", function (event, data) {
                    schema.addValue($("[name=" + _replaceMetacharator($fileInput.data("displayId")) + "]", $form), data, preview);
                });
                $fileInput.on("removefile", function (event, data) {
                    schema.removeValue($("[name=" + _replaceMetacharator($fileInput.data("displayId")) + "]", $form), data, preview);
                });
                ...
                $fileInput.fileupload({
                    ...
                    add: function (e, data) {
                        ...
                        data.fileInput.trigger("addfile", result);
