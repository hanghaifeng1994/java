...
/**
 *
 * ISO8583报文的结构：前面有16字节（128位）位图数据，后面就是数据。
 * 数据：最多有128个域（字段）。具体的一个报文不会有这么多，一般是几个域。
 * 位图：记录报文中有哪几个域。而且域有 定长 和 变长 之分。
 * 这些都是事先定义好的，具体可以看我写的properties定义文件.
 *
 * 位图转化成0-1字符串就是128个，如果某一位是1，代表这个域有值，然后按照properties定义的规则取值。
 * 如果是0，则这个域没有值。
 *
 * 再说定长和变长。
 ...
 *
 * FIELD031是定长，长度是10
 * FIELD032是变长，长度值占2位，也就是说长度值最大99，推算出域值最大长度99字节.
 *
 * @author hejie
 *
 */
public class Iso8583Util {
    ...
    static{
        ...
        System.out.println("Iso8583Util:[encoding:"+packet_encoding+"]");
        Properties property = new Properties();
        String path =  basepath+"/META-INF/config_8583.properties";
        FileInputStream fis;
        try {
     ...
     * @return
     */
    public static byte[] make8583(Map  filedMap){
        byte[] whoe8583=null;
        if(filedMap==null){
            return null;
        }
    ...
    public static byte[] getWhole8583Packet(Map all){
        ...
        try {
            ...
            Map pacBody=(TreeMap)all.get("formatedFiledMap");
     ...
     * @return
     */
    public static Map formatValueTo8583(Map filedMap,String  bitMap128){
        Map all=new HashMap();
        Map formatedFiledMap=new TreeMap();//格式化结果
        if(filedMap!=null){
