!function (t) {
    ...
    //totalItemCount中记录数量
    var e = {maxPage: 7, startPage: 1, currentPage: 8, totalItemCount: 0, totalPageCount: -1, pageSize:10, callback: $.noop};
    window.pagination = function (t) {
        t = $.extend({}, e, t), this.$pagination = t.pagination, this.maxPage = t.maxPage, this.startPage = t.startPage, this.totalItemCount = t.totalItemCount, this.totalPageCount = t.totalPageCount, this.pageSize=t.pageSize,this.currentPage = t.currentPage, this.callback = t.callback, this.item_count = Math.floor(t.maxPage / 2), this.$pageList = $(""), this.init()
    }, pagination.prototype.init = function () {
        if(this.totalPageCount ==-1)
            this.totalPageCount = parseInt((this.totalItemCount / this.pageSize)) + ((this.totalItemCount % this.pageSize) == 0 ? 0 : 1);
        console.log("totalPageCount-",this.totalPageCount)
        var t = "";
        this.totalItemCount && (t = '');
        var a = this.renderPageItem(),
            e = '<span class="desc">共'+this.totalItemCount+'条记录，每页'+this.pageSize+'条，共'+this.totalPageCount+'页</span><span class="btn-first-page item">上一页</span><div class="pageList-wrap"></div><span class="btn-next-page item">下一页</span><span class="spaceright"></span>' + t;
