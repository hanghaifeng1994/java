...
public class LeaveService extends BaseService {
	...
	public Leave get(String id) {
		...
		HistoricProcessInstance historicProcessInstance = historyService.createHistoricProcessInstanceQuery().processInstanceId(leave.getProcInsId()).singleResult();
		if(historicProcessInstance!=null) {
			...
			variables = runtimeService.getVariables(runtimeService.createProcessInstanceQuery().processInstanceId(leave.getProcInsId()).active().singleResult().getId());
		}
...
//				ActUtils.PD_LEAVE[0], businessKey, processInstance.getId(), variables });
	...
	public Page<Leave> find(Page<Leave> page, Leave leave) {
		...
		for(Leave item : page.getList()) {
			String processInstanceId = item.getProcInsId();
