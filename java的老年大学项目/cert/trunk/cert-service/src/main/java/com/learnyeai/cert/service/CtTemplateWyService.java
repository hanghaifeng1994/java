...
public class CtTemplateWyService extends WeyeBaseService<CtTemplate> {
	...
	public Map<String, Object> deleteTp(CtTemplate tp) {
		...
		int num = 0;
		for (String tpId : tp.getTpId().split(",")) {
			CtTemplate delOne = new CtTemplate();
			delOne.setDelFlag("1");
			delOne.setTpId(tpId);
			int c = ctTemplateMapper.updateByPrimaryKeySelective(delOne);
			if(c>0) num++;
		}
		result.put("status", "0");
		result.put("num", num);
	...
	protected Weekend<CtTemplate> genSqlExample(CtTemplate t, Map params) {
		...
		if (StringUtils.isNotBlank(t.getSiteId())) {
			c.andIn(CtTemplate::getSiteId, Arrays.asList(t.getSiteId().split(",")));
		}
		if(t.getSiteIds()!=null) {
			c.andIn(CtTemplate::getSiteId, t.getSiteIds());
		}
		w.and(c);
