...
<%@ include file="/common/admin_meta.jsp"%>
	...
	<script type="text/javascript">
		function jumpToPersonCount(cityid,areaid){
			$("#programeID").val($("#programesId").val());
			$("#cityId").val(cityid);
			$("#cantonId").val(areaid);
			$("#mainForm3").submit();
		}
...
<div class="bs-example" > 
	<ul class="nav nav-tabs nav-justified" id="tab" style="padding: 0px;">
        <li class="active"><a style="cursor: pointer;" href="${ctx }/count/manage/learningsituation.action">地区学情统计</a></li>
        <li><a style="cursor: pointer;" href="${ctx }/count/manage/learningsituation!highchart.action">地区图表统计</a></li>
    </ul>                
</div> 
<form class="form-inline dr-form-inline" id="mainForm" name="mainForm" action="${ctx}/count/manage/learningsituation.action" method="post">
...
<span class="glyphicon glyphicon-search"></span>
  搜索
	...
	<form class="form-inline dr-form-inline"  id="mainForm2" name="mainForm2" action="${ctx}/count/manage/learningsituation!list.action"
...
<table class="table table-bordered dr-table-default">
 ...
 <s:iterator value="learninglist" status="stat">
		<tr>
		<td width="80px;"><a href="#" onclick="jumpToPersonCount('${cityid}','${cantonid }');">${areaname}</a></td>
...
<form id="mainForm3" name="mainForm3" action="${ctx}/count/manage/studentcount.action" method="post" enctype="multipart/form-data">
	<input type="hidden" name="programesId" id ="programeID" />
	<input type="hidden" name="cityId" id ="cityId" />
	<input type="hidden" name="cantonId" id ="cantonId" />
</form>
</div>
