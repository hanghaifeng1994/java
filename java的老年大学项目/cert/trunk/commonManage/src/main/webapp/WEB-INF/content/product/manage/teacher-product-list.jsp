...
<title>辅导老师辅导的书籍<s:if test="currentTenant!=null">-${currentTenant.logo}</s:if><s:else><%@ include file="/common/title.jsp" %></s:else></title>
...
<div class="dr-searchbar" style="text-align: left;">
  <form class="form-inline dr-form-inline" id="mainForm" name="mainForm" action="${ctx}/product/manage/teacher-product.action" method="post">
	...
	<input type="hidden" name="curRoleId" id="curRoleId" value="${curRoleId}" />	
	 <div class="form-group">
	    <label>书籍名称</label>
	      <input name="product.name" type="text" class="form-control input-sm" value="${product.name}" />
	    </div>
	    <div class="form-group">
	    <label>书籍代码</label>
	      <input name="product.serialNo" type="text" class="form-control input-sm" value="${product.serialNo}"/>
	      </div>
	    <div class="form-group">
    ...
    <tr>
      ...
      <td>${studyNum}</td>
       ...
       <div id="hpn" align="left" style="display: none;width: 80%;float: left" onclick="hidediv(this);">
          	...
          	<!--<a target="_blank" href="${ctx }/product/manage/teacher-product!learningcount.action?clazzid=${id}&curRoleId=${curRoleId}">查看${name}的学情统计</a>&nbsp;&nbsp;&nbsp;(<a target="_blank" href="<common:prop name="fang.main.url" propfilename="sysconfig.properties"/>/fang/fang!index.action?fangId=${fangId}">进入研修坊</a>)<br></br>
       ${name}
        -->
 </td>
      ...
      <td>
        <s:if test="newlp">
		<a href="#" onclick="lpoperaction(${id})">书籍辅导</a>
		</s:if>
		<s:else>
		<select name="oper" onchange="operaction(this,${id})">
			<option value="">选择操作</option>		
			<option value="product!examPaperRule.action?id=${id}">书籍测验管理</option>
			<option value="product!examination.action?id=${id}">测验阅卷</option>
			<option value="product!addHomeWorks.action?id=${id}">作业布置</option>
			<option value="product!readHomeWorks.action?id=${id}">作业批阅</option>				
		</select>
		</s:else>
		<s:if test="#request.iswxeduopen=='true'">
			<%-- <a href="${ctx }/clazzproduct/manage/clazzproduct!teacherQuestion.action?productId=${id}">在线提问</a> --%>
...
<h1 align="center"><font color="#333333">您暂无管理的书籍</font></h1>
</div>
</div>
</div>
</s:else>
