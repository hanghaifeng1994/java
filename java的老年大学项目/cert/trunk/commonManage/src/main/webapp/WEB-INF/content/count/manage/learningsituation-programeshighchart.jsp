...
<div class="dr-wrapper">
	...
	<jsp:param value="programesCount" name="menu" />
...
<div class="bs-example" > 
	<ul class="nav nav-tabs nav-justified" id="tab" style="padding: 0px;">
        <li><a style="cursor: pointer;" href="${ctx }/count/manage/learningsituation!programes.action">项目学情统计</a></li>
        <li class="active"><a style="cursor: pointer;" href="${ctx }/count/manage/learningsituation!programeshighchart.action">项目图表统计</a></li>
    </ul>                
</div> 
...
<div class="form-group" id="highchart_type" style="float: left;"> 
	    ...
	    <button class="btn btn-sm " type="button"   onclick="loadHighchart(6,3,'总项目')" id="highchart_6">24小时人数</button>
		<button class="btn btn-sm " type="button"   onclick="loadHighchart(7,4,'总项目')" id="highchart_7">单日学习时长</button>
	    &nbsp; 单日时间设置：    
...
function loadHighchart(type,htype,typename){
	...
	if(htype ==1){
	    ...
	    chart = new Highcharts.Chart({    
            chart: {    
		        ...
		        series: loadProgrames24HourData(typename)
		    }); 
	}else if(htype ==4){  
	    ...
	    chart = new Highcharts.Chart({
            ...
            series: loadProgramesDateHourData(typename)
        }); 
	}
}
    ...
    function loadProgramesDateHourData(typename){
    	var sdata = new Array(); 
		$.post("${ctx }/count/manage/learningsituation!loadProgramesDateHourData.excsec?typeName="+typename+"&startTime="+Date.parse($("#startTime").val())+"&endTime="+Date.parse($("#endTime").val()),function(data) {
	...
	function loadProgrames24HourData(typename){
		var sdata = new Array(); 
		$.post("${ctx }/count/manage/learningsituation!loadProgrames24HourData.excsec?typeName="+typename,function(data) {
			var arrayObj = data.toString().split(",");　
			$(arrayObj).each(function(index){   
				sdata.push(parseInt(arrayObj[index]));  
			});
	...
	function initData(){ 
	 	$.post("${ctx }/count/manage/learningsituation!getprogramesHighchartData.excsec",{tenantId:$("#tenantId").val()},function(data) {
