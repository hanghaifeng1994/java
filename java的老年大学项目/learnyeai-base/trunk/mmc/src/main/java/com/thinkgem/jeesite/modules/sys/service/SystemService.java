...
public class SystemService extends BaseService implements InitializingBean {
	...
	public List<User> findUserByOfficeId(String officeId) {
		List<User> list = (List<User>)CacheUtils.getCacheVal(UserUtils.USER_CACHE, UserUtils.USER_CACHE_LIST_BY_OFFICE_ID_ + officeId);
		if (list == null){
			User user = new User();
	...
	public void saveUser(User user) {
		if (StringUtils.isBlank(user.getId())){
			...
			if (oldUser.getOffice() != null && oldUser.getOffice().getId() != null){
				CacheUtils.getCache(UserUtils.USER_CACHE).evict(UserUtils.USER_CACHE_LIST_BY_OFFICE_ID_ + oldUser.getOffice().getId());
			}
			// 更新用户数据
			user.preUpdate();
			userDao.update(user);
...
//		systemRealm.clearAllCachedAuthorizationInfo();
		// 清除日志相关缓存
		CacheUtils.evict(LogUtils.CACHE_MENU_NAME_PATH_MAP);
	}
